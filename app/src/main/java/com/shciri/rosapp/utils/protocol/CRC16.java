...
public class CRC16 {
    ...
    public static int calc(byte[] data, int offset, int len) {
        return calc(data, offset, len, 0xffff);
    }

    public static int calc(byte[] data, int offset, int len, int preval) {
    ...
    public static int calc_crc16(byte[] data, int len) {
        int crc16High = 0x00ff;
        int crc16Low = 0x00ff;
        int Index = 0;

        for (int i=0; i < len; i++)
        {
            Index = (crc16Low ^ (byte)data[i]) & 0x00ff;
            crc16Low = crc16High ^ crc16_tab_h[Index];
            crc16High = crc16_tab_l[Index];
        }

        return (crc16High  << 8 | (crc16Low & 0x00ff)) & 0xffff;
    }

    public static void formatFrameBuffer(byte[] data) {
        data[3] = (byte)(data[0] + data[1] + data[2]);

        int len = ((data[1] | data[2] << 8) & 0x3FF);

        int crc16 = CRC16.calc_crc16(data, len - 2);

        data[len - 2] = (byte)(crc16 & 0x00FF);
        data[len - 1] = (byte)(crc16 >> 8);
    }
}
